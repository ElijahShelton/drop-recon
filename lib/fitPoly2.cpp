...
{ size_t n = ((struct data *)data)-> n;
    ...
    double p20 = gsl_vector_get(x, 0);
    double p02 = gsl_vector_get(x, 1);
    double p11 = gsl_vector_get(x, 2);
    double p10 = gsl_vector_get(x, 3);
    double p01 = gsl_vector_get(x, 4);
    double p00 = gsl_vector_get(x, 5);
    ...
    for (i = 0; i < n; i++)
    {
        ...
        double tx = SX[i];
        double ty = SY[i];
        double Zi = p20*tx*tx+p02*ty*ty+p11*tx*ty+p10*tx+p01*ty+p00;
...
modelFunction_df (const gsl_vector * x, void *data, gsl_matrix * J)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        /* Zi = p20 x^2 +p02 y^2+p11 x y + p10 x +p01 y + p00*/
        /* and the xj are the parameters (p20, p02,p11,p10,p01,p00) */
        double tx = SX[i];
        double ty = SY[i];
        double tw = weights[i];
        
        gsl_matrix_set (J, i, 0, 1/tw*(tx*tx));
        gsl_matrix_set (J, i, 1, 1/tw*(ty*ty));
        gsl_matrix_set (J, i, 2, 1/tw*(tx*ty));
        gsl_matrix_set (J, i, 3, 1/tw*(tx));
        gsl_matrix_set (J, i, 4, 1/tw*(ty));
        gsl_matrix_set (J, i, 5, 1/tw*(1));
...
double *fPoly (double fitParInitial[],double dataX[],double dataY[], double dataZ[],double weights[],int dimData,int dimfpar)
{
    ...
    //put number of fit parameters here
    const size_t p = 6;
   ...
   // mexPrintf("status = %s\n", gsl_strerror (status));
    ...
    //mexPrintf("p20 = %.5f +/- %.5f\n", FIT(0), ERR(0)*chi / sqrt(dof));
    int lfitPar=p;
    //generate results: 0-5 fitparameters 5
    
    static double fitResults[13]={0.0};
    iter=0;
    //for errors of the fitparameter see 
    //http://www.gnu.org/software/gsl/manual/html_node/Fitting-Overview.html
    do {
        fitResults[iter]=gsl_vector_get(s->x, iter);
        //factor 2 yields 2 sigma confidence intervals for fit parameters
        fitResults[iter+lfitPar]=2*c*ERR(iter);
        iter++;
    } while (iter<lfitPar);
...
{
    mexPrintf ("iter: %3u x = % 15.8f % 15.8f "
            "|f(x)| = %g\n",
            iter,
