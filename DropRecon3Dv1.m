classdef DropRecon3Dv1
    ...
    %
    % Copyright (c) 2017,	University of California, Santa Barbara
    %                       All rights reserved.
    %
    % License:              Open Source under BSD
    % Contact:              shelton.elijah@gmail.com
    % Coded by:             Elijah Shelton
    ...
    methods (Static)
        function [] = readme()
            readme_str = ['DropRecon Version 1.0 // 17 July 2017 \n',...
                ...
                'Elijah Shelton at MATLAB Central or via email: \n',...
                'eshelton@engr.ucsb.edu\n',...
                '\n',...
                'This code was developed by Elijah Shelton at UC Santa Barbara,\n',...
                'based on work by Friedhelm Serwane, with input from Otger Campas.\n',...
        ...
        function [] = compileMex()
            ...
            if ismac % If running on a mac
                ...
                if ~exist(lsp,'dir') || ~exist(hsp,'dir')
                    ...
                    fprintf('2) In the terminal, type: brew serach gsl \n')
                    fprintf('3) In the terminal, type: brew install gsl@1 \n')
                    fprintf('4) In the matlab prompt, type: DropRecon3Dv1.compileMex \n')
                end
        ...
        function [] = compileMexOld()
            ...
            %mex(lPath,iPath,'-lgslcblas','-lgsl','lib/fitGauss.cpp');
            %mex(lPath,iPath,'-lgslcblas','-lgsl','lib/fitPoly1P10P01P00.cpp');
            %mex(lPath,iPath,'-lgslcblas','-lgsl','lib/fitPoly2X.cpp');
            %mex(lPath,iPath,'-lgslcblas','-lgsl','lib/fitPoly2.cpp');
        ...
        function [drop] = saveDrop(meas,defPar)
            ...
            save(drop.path2ResultsFile,'drop','defPar');
        end
        ...
        function [image] = defPar2Image(defPar)
            ...
            %             for i = 1:nVirtualStacks
            %                 imshow(image.interpDbl(:,:,i)./max(image.interpDbl(:)))
            %                 pause(0.01)
            %             end
            ...
            switch recon.message
                ...
                case 'Successful reconstruction of coordinates!';
                    ...
                    if measurement.findCurvs
                        ...
                        %FOM = (measurement.H_rec-...
                        %        measurement.H_anal)./measurement.H_anal;
                        FOM = measurement.GOF.p;
                        measurement.FOM_hist=FOM;
                        noi = measurement.numOfIterations;
                        %H.RE95CI=nan(noi+1,1);
                        %H.REmean=nan(noi+1,1);
                        %H.REstd=nan(noi+1,1);
                        ...
                        while measurement.continue
                            ...
                            measurement.iterNum=i;
                            suffix = ['Iter',num2str(i)];
                            %                             measurement.relErrH=(measurement.H_rec-...
                            %                                 measurement.H_anal)./measurement.H_anal;
                            %                             measurement.relErrMeanCurvAVERAGE = ...
                            %                                 mean(measurement.relErrH(~isnan(...
                            %                                 measurement.relErrH)));
                            %                             measurement.relErrMeanCurvSTDEV= ...
                            %                                 std(measurement.relErrH(~isnan(...
                            %                                 measurement.relErrH)));
                            %                             measurement.relErrMeanCurv95CI = ...
                            %                                 quantile(abs(measurement.relErrH),0.95);
                            
                            measurement.FOM = measurement.GOF.p;
                            %H.RE95CI(i) = measurement.relErrMeanCurv95CI;
                            %H.REmean(i) = measurement.relErrMeanCurvAVERAGE;
                            %H.REstd(i) = measurement.relErrMeanCurvSTDEV;
                            %assignin('base','H',H)
                            ...
                            %                             plot(H.REmean,'.-g')
                            %                             plot(H.REstd,'.-b')
                            %                             plot(sum(measurement.GOF.h)/length(measurement.GOF.h),'.k')
                            %                             legend('95%','Mean','Std','Poor Fits')
                            %                             print(gcf,[measurement.path2ResultsDir,...
                            %                                 'EffectOfIterations'],'-depsc');
                            ...
                            %measurement = DropRecon3Dv1.makeAnimatedGifs(...
                            %    measurement,suffix);
                            %save([measurement.path2ResultsFile(1:end-4),...
                            %    suffix,'.mat'],'measurement');
                        ...
                        %% Calculate Relative Error of Curvature Measurements
                        %                         measurement.relErrH=(measurement.H_rec-...
                        %                             measurement.H_anal)./measurement.H_anal;
                        %                         measurement.relErrMeanCurvAVERAGE = ...
                        %                             mean(measurement.relErrH(~isnan(...
                        ...
                        %                             quantile(abs(measurement.relErrH),0.95);
                        %
                        %                         measurement.relErrH_ani=(measurement.H_rec_ani-...
                        %                             measurement.H_anal_ani)./measurement.H_anal_ani;
                        %                         measurement.relErrMeanCurv_ani_AVERAGE = ...
                        %                             mean(measurement.relErrH_ani(~isnan(...
                        %                             measurement.relErrH)));
                        %                         measurement.relErrMeanCurv_ani_STDEV= ...
                        %                             std(measurement.relErrH_ani(~isnan(...
                        %                             measurement.relErrH_ani)));
                        %                         measurement.relErrMeanCurv_ani_95CI = ...
                        %                             quantile(abs(measurement.relErrH_ani),0.95);
        ...
        function [measurement] = makeAnimatedGifs(measurement,suffix)
            noGif = 1;
            if nargin==1
                suffix='';
            end
            TS = datestr(clock);
            TS = [TS(1:11),'_',TS(13:14),'h_',TS(16:17),'m_',TS(19:20),'s'];
            suffix=[suffix,'-',TS];
            suffix = '';
            %% Recon Radius Map
            X = measurement.xGrid+measurement.center(1);
            Y = measurement.yGrid+measurement.center(2);
            Z = measurement.zGrid+measurement.center(3);
            V = measurement.rGrid;
            titStr = 'Surface Map of Recon Radii [px]';
            gifPath = [measurement.path2ResultsDir,...
                'ReconRadiiMap',suffix,'.gif'];
            if noGif
            else
                DropRecon3Dv1.makeGif(X,Y,Z,V,titStr,gifPath);
            end
            %% Rel Err of Radius Map
            V = (measurement.rGrid - measurement.r_analGrid)./...
                measurement.r_analGrid;
            titStr = 'Surface Map of Rel Err Radii [px]';
            gifPath = [measurement.path2ResultsDir,...
                'RelErrRadiiMap',suffix,'.gif'];
            if noGif
            else
                DropRecon3Dv1.makeGif(X,Y,Z,V,titStr,gifPath);
            end
            %% Abs Err of Radius Map
            V = (measurement.rGrid - measurement.r_analGrid);
            titStr = 'Surface Map of Abs Err Radii [px]';
            gifPath = [measurement.path2ResultsDir,...
                'AbsErrRadiiMap',suffix,'.gif'];
            if noGif
            else
                DropRecon3Dv1.makeGif(X,Y,Z,V,titStr,gifPath);
            end
            %% Analytical Radius Map
            V = measurement.r_analGrid;
            titStr = 'Surface Map of Analytical Radii [px]';
            gifPath = [measurement.path2ResultsDir,...
                'AnalyticalRadiiMap',suffix,'.gif'];
            if noGif
            else
                DropRecon3Dv1.makeGif(X,Y,Z,V,titStr,gifPath);
            end
            %% Measured Curavature Map
            V = measurement.HGrid;
            titStr = 'Surface Map of Mean Curvature';
            gifPath = [measurement.path2ResultsDir,...
                'MeasuredCurvatureMap',suffix,'.gif'];
            if noGif
            else
                DropRecon3Dv1.makeGif(X,Y,Z,V,titStr,gifPath);
            end
            %% Rel Err of Curv Map
            measurement.relErrHGrid = (measurement.HGrid -...
                measurement.H_analGrid)./measurement.H_analGrid;
            
            V = measurement.relErrHGrid;
            titStr = 'Surface Map of Rel Err of Mean Curvature';
            gifPath = [measurement.path2ResultsDir,...
                'RelErrCurvatureMap',suffix,'.gif'];
            if noGif
            else
                DropRecon3Dv1.makeGif(X,Y,Z,V,titStr,gifPath);
            end
            %% Abs Err Curv Map
            measurement.absErrHGrid = (measurement.HGrid -...
                measurement.H_analGrid);
            
            V = measurement.absErrHGrid;
            
            titStr = 'Surface Map of Abs Error of Mean Curvature';
            gifPath = [measurement.path2ResultsDir,...
                'AbsErrCurvMap',suffix,'.gif'];
            ...
            %% Analytical Curv Map
            V = measurement.H_analGrid;
            
            titStr = 'Surface Map of Analytical Mean Curvature';
            gifPath = [measurement.path2ResultsDir,...
                'AnalyticalCurvMap',suffix,'.gif'];
        ...
        function [] = makeProjectionMap(measurement,suffix)
            if nargin==1
                suffix='';
            end
            X = (pi/2-measurement.elv).*cos(measurement.azi);
            Y = (pi/2-measurement.elv).*sin(measurement.azi);
            Z = (measurement.H_rec-measurement.H_anal)./measurement.H_anal;
            scatter(X,Y,[],Z,'.')
            F_Z = scatteredInterpolant(X,Y,Z,'natural');
            [XGRID,YGRID] = meshgrid(linspace(-pi/2,pi/2,314));
            ZGRID = F_Z(XGRID,YGRID);
            RGRID = sqrt(XGRID.^2+YGRID.^2);
            projMap = ZGRID;
            projMap(RGRID>pi/2) = nan;
            close all
            figure(1)
            imshow(projMap,[])
            colormap(measurement.cmap)
            
            colorbar
            axis equal
            measurement.path2CurvErrorMap = ...
                [measurement.path2ResultsDir,...
                'PolarProjRelErrH',suffix];
            print(gcf, measurement.path2CurvErrorMap,'-djpeg');
            
            %caxis([-0.5 0.5])
            %print(gcf,[measurement.path2CurvErrorMap,'50pct'],'-djpeg');
            
            %% Arc Length
            close all
            [s_anal,ind] = sort(measurement.s_anal);
            relErrH = measurement.relErrH(ind);
            H_anal = measurement.H_anal(ind);
            %dHds_anal = measurement.dHds_anal(ind);
            plot(s_anal,relErrH,'.r')
            hold on
            plot(s_anal,relErrH.*H_anal,'.b')
            hold on
            plot(s_anal,H_anal,'.k')
            %hold on
            %plot(s_anal,dHds_anal,'.g')
            legend('Rel Err H', 'Abs Err H','H analytical',...
                'Location','SouthEast')
            axis([0 max(measurement.s_anal) -1 1])
            fname = [measurement.path2ResultsDir,...
                'ArcLenRelErrH',suffix];
