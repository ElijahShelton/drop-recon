...
{ size_t n = ((struct data *)data)-> n;
    ...
    double p11=gsl_vector_get(x, 1);
    double p10=gsl_vector_get(x, 2);
    double p01=gsl_vector_get(x, 3);
    double p00=gsl_vector_get(x, 4);
    ...
    for (i = 0; i < n; i++)
    {
        ...
        double tx = SX[i];
        double ty = SY[i];
        double Zi = p20*tx*tx+p11*tx*ty+p10*tx+p01*ty+p00;
...
modelFunction_df (const gsl_vector * x, void *data, gsl_matrix * J)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        /* Zi = p20 x^2 +p11 x y + p10 x +p01 y + p00*/
       ...
       // gsl_matrix_set (J, i, 1, 1/tw*(2*p02*ty));
        gsl_matrix_set (J, i, 1, 1/tw*(tx*ty));
        gsl_matrix_set (J, i, 2, 1/tw*(tx));
        gsl_matrix_set (J, i, 3, 1/tw*(ty));
        gsl_matrix_set (J, i, 4, 1/tw*(1));
...
double *fPoly (double fitParInitial[],double dataX[],double dataY[], double dataZ[],double weights[],int dimData,int dimfpar)
{
    ...
    static double fitResults[11]={0.0};
    iter=0;
    do {
